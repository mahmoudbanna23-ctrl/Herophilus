# Codex brief — build and finish HALF A of topic 10 drafts (Grade Gain neurology, "Movement Disorders")
...
Unlike topic 09, **no draft file exists yet for topic 10** — you are creating half A from the
staging file directly, not merely expanding an existing draft.

## Your half
**Half A: Q199–Q213**, ids `npqb-nr-199` … `npqb-nr-213` (15 entries). Create
`content/neuro/qb-pages/gg-nr-t10.draft-A.js`. Half B (Q214–Q227) is a separate job on a separate file
— do not touch it.
...
- Q205: stem starts lowercase `"which is an early sign..."` — keep lowercase.
- Q203: stem omits a linking word after "disease" (`"...man with Parkinson's disease cannot
  tolerate..."`) — keep as printed, do not insert "who".
- Q199: the box says "right side" while the stem says "left arm" — a genuine printed contradiction.
  Keep both exactly as printed; do not resolve it in `stem`/`options`/box quote. You will note it in
  `explanation` (see below) — record, never correct, per house rule.
- Q212: only 3 options printed (no D/E) — keep as 3.
...
- Options text elsewhere may carry OCR-plausible printed spellings (e.g. Q204's "Cauda equine")
  — these are frozen `options` text; copy exactly, never correct.

## Boxed entries — Q199, Q200, Q202, Q203, Q211 (5 of your 15)
...
**no marker sentence**. The other 10 entries in your half (201, 204–210, 212, 213) are unboxed.
...
2. Write each `explanation` to the **writing budget**, measured in words:
   ...
   - Vignettes in this half: Q199, Q200, Q202, Q203, Q211 (also boxed), Q208, Q214-range not yours.
     Use judgement — a short stem testing one fact is 250-class; a clinical scenario or a printed
     defect needing discussion is 520-class.
3. **Outside knowledge rule**: any claim not contained in `L3) MOVEMENT DISORDERS.txt` gets the
   ...
   worse than a tag.
4. **Q199's laterality contradiction**: in `explanation`, note plainly that the printed box says
   "right side" while the stem describes a left-arm tremor, and that this is preserved as printed,
   not resolved — do not silently pick one side.
5. **Marker**: every unboxed entry's `explanation` ENDS with exactly
   ...
   `source`. Expected: 10 markers in this half (201, 204–210, 212, 213).
...
- Syntax: `node -e "const fs=require('fs');const s=fs.readFileSync(process.argv[1],'utf8');const A=eval('['+s+']');console.log(A.length)" "content/neuro/qb-pages/gg-nr-t10.draft-A.js"` — must print 15.
- For each entry print: id, word count of `explanation`, ends-with-marker yes/no, answer index used.
  Count markers (expect 10).
- Confirm emoji count 0. Confirm the 5 boxed entries' box text appears verbatim in their
  `explanation`.
...
count, emoji count, non-ASCII character scan result, confirmation the Q199 contradiction is noted in
its explanation, confirmation `keyName`-cross-check was NOT performed (none available) and this is
stated rather than silently skipped. List anything you were unsure about. Do not claim done for
