...
| ✅✅✅ **`nr-delirium` + `nr-coma` + `nr-comapproach`** | **657** deck (L13 delirium half) **+ 5,374 book** (Neurology ch.11, printed 99–118) = **6,031** | ÷280 said **21.5 pp** | ✅✅ **ALL THREE WRITTEN 2026-08-23 — `nr-delirium` 10 sections, 3,477 w, printed 11 pp, 316 w/pp · `nr-coma` 9 sections, 3,020 w, printed 10 pp, 302 w/pp · `nr-comapproach` 7 sections, 3,823 w, printed 12 pp, 319 w/pp.** All 0 q. **THE MODULE WENT 34 → 36.** ✅✅ **THE COMA RENDER, ON THE USER’S EXPLICIT AUTHORISATION:** book **Neurology ch.11, `CLINICAL APPROACH TO “COMA” CASE`, printed 99–118 = PDF 110–129, TWENTY pages**, rendered at 150 dpi and read by **two subagents**, cached to `content/neuro/book/` as **`neuro-ch11-coma-a.txt`** (99–108, 2,535 w) and **`neuro-ch11-coma-b.txt`** (109–118, 2,839 w). ⚠️ **The `+11` offset was verified FOOTER BY FOOTER on ALL TWENTY pages; NOTHING was illegible.** ⚠️⚠️ **DO NOT RE-RENDER.** ⚠️⚠️ **TWO SPLITS, AND THEY WERE TAKEN ON DIFFERENT EVIDENCE.** **(1) Delirium against coma — forecast, and it needed no arithmetic: TWO TOPICS, TWO SOURCES, CONTIGUOUS.** **(2) Coma into two — ⚠️⚠️ TAKEN ON A *MEASURED WRITTEN COUNT*, NOT AN ESTIMATOR.** The chapter was **written whole and measured 6,543 body words**; at **every w/pp rate this module has recorded (296–391) that is 17–22 pp**, so **there was no rate at which it fitted the shape.** **It printed 10 + 12 = 22, at the top of that band.** ✅ **THE SEAM WAS THE BOOK’S OWN BANNER — `APPROACH TO THE PATIENT IN COMA`, printed p.106.** ✅✅ **`nr-comapproach` PAYS TWO ROWS NO DECK COULD:** ⚠️ **THIAMINE, QUANTIFIED AT LAST — *at least 100 mg IV, WITH 50 % dextrose*** (`ca-1`), where **`ps-sud` `sd-8`, `ps-geri` `ge-5` and `nr-delirium` `dl-3` all name Wernicke–Korsakoff and NONE gives a dose** · and **NALOXONE** (`ca-6`), where **`sd-8` records `naloxone` returns ZERO across all 25 decks.** ✅ **AND THE INVESTIGATION PANEL `dl-7` records as missing** — L13 names no investigation at all for delirium; `ca-6` prints it. ⚠️⚠️ **A CROSS-CHECK ON THE TWO AGENT REPORTS, RECORDED:** agent B reported **“brain-death criteria: NO” for printed 109–118**, and agent A found **brain death IN FULL on printed 103–104.** **BOTH ARE RIGHT ABOUT THEIR OWN RANGE** — B scoped its claim correctly to the pages it read. **Neither is corrected, and the criteria are in `nr-coma` `co-7`.** *The `check the auditor` rule applied to a subagent, and it passed.* ⚠️ **THE BOOK ASKS TWO QUESTIONS AND ANSWERS NEITHER** — the p.106 pontine-haemorrhage MCQ and p.118’s *The last question*. **BOTH WORKED in `ca-7`**, the `ps-psychosis` `sm-9` shape. ⚠️ **TWO CONTRADICTIONS WITHIN `L13`, BOTH RECORDED, NEITHER CORRECTED:** the **MMSE** is `ps-geri` `ge-10`’s dementia instrument and **`dl-7` says it is NOT SENSITIVE for delirium** — *same instrument, opposite verdicts, one deck* · and `dl-1` calls delirium **transient, usually reversible** while `dl-6` gives **50 % improve, 25 % never return to baseline, 25 % DIE.** ⚠️ **FIVE of the delirium half’s slides carry no usable text** (32, 34, 36, 39 none at all; one carries `FEVER`) — **all five in this half; `ps-geri`’s dementia half is clean.** ⚠️ **ONLY SEVEN QUANTIFICATIONS EXIST IN ALL OF BOOK CH.11**, listed in `ca-7`; **nothing is supplied.** ⚠️ **THALAMIC LESIONS sits on the book’s own coma differential and is never expanded** (`co-7`). ⚠️ **Book Psychiatry ch.8 (239–248) REMAINS UNREAD** — image-only, **shared with `ps-geri`**; the render covered Neurology ch.11 ONLY |
...
4. ✅ ✅✅ **WRITTEN — 36 of 36. THE MODULE IS COMPLETE** *(measured from disk 2026-08-23; the module went 33 → 34 when
   ...
   **`nr-exam`** · **`nr-cns`** · **`ps-somato`** · **`ps-child`** · **`ps-geri`** · **`ps-interview`** · **`ps-osce`** · **`nr-delirium`** · **`nr-coma`** · **`nr-comapproach`**.
   **333 sections, 113,586 body words.** ✅⚠️ **ALL 151 OF THE MODULE'S QUESTIONS ARE NOW REACHABLE
   FROM THEORY — it was 130 before `nr-intro`/`nr-exam` landed, and the remaining 21 were all in
   that one chapter.**
   ✅✅✅ **NOTHING REMAINS. `modcheck-neuro.js` reads 36 of 36 declared, 0 dead qs ids, 0 wrong-chapter links, 0 dead section refs.**
   ✅ **And the boot probe reads `chaps 153` against `th 153` — EVERY CHAPTER IN THE APP NOW HAS THEORY.**
