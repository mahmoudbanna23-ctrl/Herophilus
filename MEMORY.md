...
all 3 carried debts cleared. Close-out: `progress\resume-ophtho.md`. ⚠️ **The ophtho ENDPOINT is a separate book — 2,442 pp / 293 MB; do NOT split the PDF, split the page RANGE.** Printed page = PDF **+1** past an unpinned flip in PDF 2151–2170. ✅ **GATE 0 CLOSED 2026-09-08** — `-oph-ep` toolchain (`sec-oph.js`/`val-oph-ep.js`/`splice-oph-ep.js`/`pagecov-oph.js`, `--part 1|2`) + stubs + both `index.html` tags done, **OPH-B unblocked**. **182 live, s1-7 done** (checked+spliced+committed; s7 Sclera closed 2026-09-16, 169->182, `c16ae83`). s8-12 next, s13 deferred, s14-28 unstaged. Close-out: `progress\resume-ophtho-endpoint-A-chapters.md` tail.**
