...
all 3 carried debts cleared. Close-out: `progress\resume-ophtho.md`. ⚠️ **The ophtho ENDPOINT is a separate, UNSTARTED book — 2,442 pp / 293 MB, the largest of the four; do NOT split the PDF (splits come out bigger), split the page RANGE, and note NO ophtho toolchain exists in `bank-harness` (all `-pd`). Conditions: `progress\ophtho-endpoint-scoping-2026-09-06.md`. ✅ MAPPED 2026-09-07 — 28 sections + ⚠️ printed page = PDF **+1** from somewhere in PDF 2151–2170 on (flip not pinned): `progress\ophtho-endpoint-section-map-2026-09-07.md`. SEAM = PDF 1390|1391 (chapters | exams). Launch two fresh Fable chats from `progress\resume-ophtho-endpoint-A-chapters.md` and `…-B-exams.md`; A builds the shared `-oph-ep` toolchain + both `index.html` tags at gate 0, B is blocked until then.**
