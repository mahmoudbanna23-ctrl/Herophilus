# Shared brief — drafting app entries from a peds ENDPOINT **PART 2** staging record

Sections 1–17 of `Pediatrics endpoint part2.pdf`. This is the **part 2** counterpart of
`pd-ep-draft-brief.md` (endpoint part 1, another live chat) and of `pd-draft-brief.md`
(Chat B's House brief) — **do not read either of those. The House one sets
...
Your work is checked by `node tools\bank-harness\val-pd-ep2.js <section> <A|B>`. **Run it yourself
...
⚠️ **YOUR OWN ESTIMATE OF YOUR STEP COUNT IS UNRELIABLE — measured five times, 2026-09-04.** A part-2
...
here, not the number. ⚠️ **A TALLY HELPS BUT DOES NOT FIX IT.** Kept-as-you-go tallies
were right three times (55, 18, and 63 against 66) — then a section 3 staging agent kept one,
said so, itemised the three calls it wasted on a mistake, and still reported **51** against a
harness-recorded **83**: 39% low while doing everything asked. So the instruction is not
"tally and you will be right". It is: tally, say it is a tally, and treat your own number as a
**floor**. If the step budget matters, stop earlier than your count says you need to. An
earlier version of this paragraph claimed the tally had solved it; the next agent disproved
that, and the claim is corrected here rather than quietly dropped.
...
chat's live file, and their duplicate guard lets a `pedep2-` id straight through. Your tools are
`val-pd-ep2.js` and `splice-pd-ep2.js`, and you never run the splicer at all — the parent does.
...
- `id`: the prefix in your task prompt plus the staged `n` — e.g. `pedep2-nd-1`.
...
agent against `a part-1 draft`). A `/* … */` provenance header at the top is
...
## 5. Length — the budget is real and part 1's first section blew it
...
⚠️ **Part 1's section 1 ran to a mean near 700 words across 89 questions.** That is the thing to
correct, not to copy — and it is a measurement carried over from the other book, not a claim about
anything in part 2. Take the *shape* from any finished entry — the lead line, the blockquote, the
rule, the distractor sweep, the closing marker — and **not the length.** Part 2 has no finished
section yet, so where you have no model, follow the shape described here rather than hunting for one
in a file this stream does not own. The validator prints a per-entry
...
**Name a sibling by its id in backticks** — `` `pedep2-nd-7` `` — never by its staging number. The
...
```
Pediatrics endpoint part2.pdf p.<the staged p>
```
...
```
Pediatrics endpoint part2.pdf p.<the staged p> (the explanation box is printed alone on p.<box>)
```
