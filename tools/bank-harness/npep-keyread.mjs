...
const PROMPT = 'This is the ANSWER page of a scanned multiple-choice question bank. One option is visibly marked as the correct answer (highlighted, coloured, boxed, ticked or underlined). ' +
  'Reply with ONLY a JSON object: {"marked":"a"|"b"|"c"|"d"|"e"|null,"printed_options":<number of options printed>,"how":"<short description of the mark>"}. ' +
  'Use null if you see no such mark; never guess from medical knowledge.';
...
for (const r of JSON.parse(fs.readFileSync(PAIRS, 'utf8'))) {
  ...
  for (let t = 0; t < 5 && !(status === 429 || (status === 200 && txt)); t++) {
    ...
    try {
      const x = await fetch('http://localhost:20128/v1/chat/completions', {
        ...
        body: JSON.stringify({ model: 'work-vision', max_tokens: 400, messages: [{ role: 'user', content: [{ type: 'text', text: PROMPT }, { type: 'image_url', image_url: { url } }] }] }),
  ...
  res[pk] = { aPage: r.a || r.q, marked: d.marked, printed: d.printed_options, how: d.how };
