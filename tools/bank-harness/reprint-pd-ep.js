// Reprint map for a peds ENDPOINT section, run BEFORE the section is drafted.
//   node tools/bank-harness/reprint-pd-ep.js <firstPage> <lastPage>
//
// Compares the OCR text of every `answered` page in the range against
//   (a) every question already live in app/data/questions.peds.ep.js  (this bank)
//   (b) every question live in app/data/questions.peds.js             (Chat B's House bank)
//   (c) the other answered pages inside the range itself              (within-section reprints)
// and prints the near matches. It RESOLVES NOTHING: (a) and (c) are folds for the drafter to
// judge, (b) is cross-bank overlap that gets RECORDED in the journal and merged once, after
// both banks close -- neither chat can fold into the other's file.
//
...
const OCR = R + 'content/peds/qb-pages/ocr/ep1/';
...
// --self-test runs section 1's own range: every one of its 89 answered pages is already live as
// a pedep-gp- entry, so anything under 89 endpoint self-matches means the matcher has drifted.
const selfTest = process.argv.includes('--self-test');
const first = selfTest ? 5 : parseInt(process.argv[2], 10);
const last = selfTest ? 210 : parseInt(process.argv[3], 10);
if (!first || !last) { console.error('usage: node reprint-pd-ep.js <firstPage> <lastPage> | --self-test'); process.exit(2); }
...
const pool = [];
live('questions.peds.ep.js', 'Q_PEDS_EP').forEach(q => q && pool.push(['endpoint ' + q.id, head(q.stem)]));
...
for (const [p, s] of mine) {
  ...
  for (const [who, ph] of pool) {
    if (near(h, ph)) { if (!selfTest) console.log('p.' + p + '  ==  ' + who); if (who.startsWith('endpoint ')) epHit.add(p); nCross++; }
  }
...
if (selfTest) {
  ...
  console.log('self-test: ' + epHit.size + '/' + pages.length + ' section-1 pages matched their live entry'
