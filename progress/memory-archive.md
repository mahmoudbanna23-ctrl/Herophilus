...
## Theory craft rules and source registers, moved out of MEMORY.md on 2026-08-23

Theory finished at 153/153, so these stopped being resume state and became reference. They are
reproduced here **verbatim, unedited**, and MEMORY.md now carries a pointer to this block.
...
## Theory war stories, moved verbatim from MEMORY.md on 2026-08-24

Theory closed at 153/153 on 2026-08-23, so these four are craft lessons for any future theory
REVISION rather than resume state. They were carried in the 200-line file until the House splice
needed the room. **Nothing was rewritten — the four bullets below are the MEMORY.md text as it
stood.**
...
## Ophthalmology theory craft, moved verbatim from MEMORY.md on 2026-08-24

Ophtho theory finished at 36 chapters and ENT theory is closed except for its two named debts, so
these are craft lessons for a REVISION, not resume state. **Nothing rewritten — the MEMORY.md text
as it stood.**
...
- **⚠️⚠️ A SOURCE'S WORD COUNT IS THE CACHE'S, NEVER `pdftotext`'s** (`op-refract`: 1,540 vs 4,439). **`pdffonts` per page detects VECTOR-OUTLINE text; zero fonts hides nothing.**
- **⚠️⚠️ VERIFY EVERY REGISTER ROW'S *SECOND CLAUSE*, AND AUDIT "row filed" CLAIMS — `op-ret` was
  ...
  Ten fault shapes + the timestamp rule: `progress\memory-archive.md`.
...
# Moved 2026-08-31 — Resume-here prune (ophtho mid-stream, user-ordered context-tax cut)

The blocks below were removed VERBATIM from `MEMORY.md`'s "Resume here" on 2026-08-31. The
resume block they came from described the state at 2026-08-30 (post-§17v). Where a rule was
still operative, a compressed one-liner stayed in MEMORY.md; this is the full record.

## The 2026-08-30 resume paragraph, whole

## Resume here — ⏳ **OPHTHALMOLOGY IS THE LIVE MODULE, ~773 BANK QUESTIONS LEFT AGAINST 27 SEP** (GG **201** = EE3–5 70 · EE6 Photos 35 · Tutorial 27 · Final 69, itemised exactly · House **~572**, estimated as ~1,025 printed − 450 written − 3 ch.16 folds netted · **endpoint RULED IN, user 2026-08-30: "finish house and GG ASAP to start Endpoint"** — endpoint ophtho starts the moment both banks close). ✅ **ENT IS CLOSED** (three banks, 30 chapters, both theory debts — `ledger.md` §15–§16, war stories in the archive). Ophtho stands at **1,330 questions** (GG **880** = topics 1–20 + End Exams 1–2 · House **450** = ch.1–16), corpus **3,891**, **105 images** — **measured 2026-08-30 by id prefix, never carried forward** (⚠️ the strict `id:'…'` regex reads 26 short — 26 entries print `id: '` with a space; count with `id\s*:\s*['"]?`); `validate.js` post-§17v: 0 BAD, 0 dead refs, 0 holes, 3,139 markers. Last boot 2026-08-30 post-§17v: **0 console errors, 876 rules, 3 `5/4` blocks, 4 modules, 153 chapters, 1,604 sections, 89 cases.** ✅ **GG End Exams 1–2 merged §17s/§17u — 29 = 29 and 27 = 27 printed (tenth and eleventh exact contents hits), numbering RESTARTS at Q1 per exam section (read off the banner), ZERO folds in both and both zeros are measured** (the End Exams reword topic questions rather than reprinting; every reject carries a named token; back-reference `opqb-t22-7` hand-read clean). ✅ **OCR gate calibrated (§17q) AND the debate verdict is IN (2026-08-30, run headless via the grok CLI — the free-lane CLIs run from Bash, no user hand-off): ADOPT-WITH-CHANGES**, six changes in `ocr-pipeline.md`'s verdict section; **the confidence half of the gate never fired in calibration and the calibration was an agreement study against our own transcript, not print** — gate stays frozen until the changes are built and re-calibrated; OpenCode tooling build launched against the pre-verdict brief, its output needs the verdict applied on top before landing. **House ch.15 "Squint" merged §17t** — 15 shipped + 2 FOLDED into `opmcq-c14-5`/`opmcq-c14-7`: **the SIXTH fold shape, within-bank cross-chapter reprint with REWORDED stem and a DROPPED fifth option** ("None of the above" — a dropped FILLER option leaves the question itself; a dropped substantive option would not: **check which option dropped before folding on this shape**). The §17r flag said "verbatim" and was right in substance, wrong in letter. Stagecheck caught three typo repairs recorded in `source` but not quoted in `explanation` (the instrument's contract) — notes added. ✅ **House ch.16 "Pediatric ophthalmology" merged §17v** — 22 shipped + 3 FOLDED (c16-12→`opmcq-c2-13`, c16-14→`opmcq-c15-8`, c16-22→`opmcq-c4-4`, all three the FIFTH shape, exact cross-chapter reprint, verified at full resolution); the [D] shared-menu trap fired again (Q22 "mucus" vs c4-3 "purulent", different keys — a pair, never a fold); four back-references hand-read clean; the resumed agent **drafted five entries from compressed memory and caught it itself against the staging record** — staging-first is what made the fault catchable. ⏸️ **SESSION PAUSED ON USER COMMAND 2026-08-30 ("finish the current task and take a break") — both in-flight batches (EE2 §17u, c16 §17v) finished and committed; NOTHING is mid-run.** On "continue": GG End Exam 3 (book pp.141–144 +1 past, 30 promised, renders READY in scratchpad `ee3\` p-148..153, classic visual flow) · House ch.17 "Ocular malignancies" (opens book p.108, renders READY `h17\` p-056..059 — ⚠️ carries three KNOWN fold candidates read at the boundary: ch.17 Q5/Q6 = ch.16 Q5/Q6 verbatim, ch.17 Q2 = ch.16 Q19 with a fifth option ADDED, "E. Choroidal melanoma" — a menu-LENGTHENED reprint). ⚠️ **THE 2026-08-29 SESSION DIED AFTER MERGING §17o (House ch.13, 27q) AND §17p (GG t20, 40q) BUT BEFORE COMMITTING THEM** — recovered and committed 2026-08-30; the interruption cost bookkeeping only, zero content. ⚠️ **`content\ophtho\book\ch20-drugs.txt` IS AN OPEN CACHING DEBT** (§17p — ch.20 read in full, the verbatim text died with the agent; a cache from memory is worse than none). ⚠️⚠️ **A DISEASE CAN BE TAUGHT IN FULL AND NEVER NAMED — GREP THE SIGN, NOT JUST THE NAME** (§17p: retinitis pigmentosa taught as "bone spicules" in `L21`, as complicated-cataract cause in `L35,36`:167, and in two book chapters, while a name-grep over the decks returned a confident absence; seventeenth absence claim overturned by opening the hit). **35 of 36 ophtho chapters seeded** (`op-appear` is the last empty one). ⚠️⚠️ **A REPAIRED BACK-REFERENCE IS A RECONSTRUCTED STEM AND MUST NOT ANSWER ITS OWN QUESTION** — seven stems and one `imgAlt` in two banks on one day named the *diagnosis* where the source printed only *findings*, two of them handing over their own key (`opmcq-c12-9`, `opqb-t16-699`). **No instrument in the chain can see it**: `stagecheck.js` compares options and keys, `sweep.js` compares stems to *other* stems, nothing compares a stem to its own key. **Read every repaired back-reference against its antecedent by hand.** ⚠️ **AN OFFSETTING ERROR IS INVISIBLE TO A SUM** — the per-bank split in `resume-ophtho.md` was wrong by exactly 37 in both directions while the total stayed right, so every sum check ever run passed. **Count the parts.** ⚠️ **`bounds.js` CANNOT MEASURE A FIGURE THAT SITS IN A COLUMN OF TEXT** (the stem above and options below are ink too, so it returns its own window's edges — which reads exactly like the clipping signature): **use a column/row density profile.** ⚠️ **FIVE FOLD SHAPES SO FAR, all within-bank, none taking an `alsoIn`** (fifth: cross-CHAPTER reprint, House Ch.1 Q4 = Ch.14 Q1, §17r)**:** exact reprint · **respelled** (mucoid/mucous — not a replaced distractor) · **REORDERED, which MOVES THE PRINTED KEY LETTER** (t9 Q392 keys B where Q355 keys C — *a key letter is never the thing to memorise*) · and one where **the second printing was BOXED and the held entry's authored marker had to come OFF**. **Check which printing is boxed before assuming a fold has no marker consequence.** ⚠️⚠️ **AND A SHARED OPTION MENU IS HOW THESE BANKS PAIR QUESTIONS, NOT EVIDENCE THEY ARE ONE:** `opmcq-c6-8` (WHO grade **TT**) and `opmcq-c6-23` (**TS**) print the same five options in the same order with **different keys**, and `sweep.js` scored the stems **1.00** because it filtered tokens on `w.length > 3` and dropped the only discriminating word in either stem. **Folding on it would have deleted the trichiasis question and left the survivor keyed for the wrong grade.** Third appearance of the short-token blindness and the worst — the earlier two made a tool skip work, this one made it return a confident wrong verdict. **Patched (capitalised short tokens survive); verified both ways — false 1.00 → 0.71, all 28 real identity matches held.** **When flagging a duplicate, NAME THE DISCRIMINATING TOKEN; if you cannot, you matched a template.** ✅✅ **THE §14.5 REGISTER IS AT ZERO OPEN ROWS, ALL FOUR MODULES** — ophtho went 39 → 0 today, and like ENT most of it was **bookkeeping, not content**. ✅✅ **ALL ELEVEN BLIND DECKS ARE CACHED — the module has no unread deck left.** **Everything is in `ledger.md` §17 — READ IT FIRST**, with `progress\resume-ophtho.md` and `content\ophtho\theory-plan.md`.

## Stale-verdict and manufactured-absence war stories (theory stream, closed)
...
## Superseded 2026-08-18 remaining-count bullet (kept for the ~304-image and model-exam facts)
...
## Pruned from MEMORY.md - 2026-09-22
