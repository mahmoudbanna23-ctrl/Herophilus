...
| `op-systemic` (L23 **l.191**, was cited ll.336–355) | **AION and the ocular motor palsies** — written here only as the vascular/syphilitic associations | `op-neuro`, `op-acute` | ☐ **VERIFIED PRE-BRIEF 2026-08-17 — PAYLOAD REAL, CITATION WRONG.** `ll.336–355` is **ocular syphilis**; AION and the motor palsies are at **l.191**, and there they are a bare **two-column caption pair inside the hypertension-associations plate** — a name each, no account. ✅ Second clause TRUE: `sys-5` and `vsc-11` carry them only as hypertension's associations, `sys-11`/`sys-12` only as the syphilis/zoster ones. **`temporal arteritis` and `ESR` return ZERO module-wide**, so the GCA workup is unwritten too. AION goes to `op-acute` (it is one of `L31,32`'s four cases); the motor palsies go to `op-neuro`. **Seventh fault shape: a row can name the right payload and the wrong line** |
