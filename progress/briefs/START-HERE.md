...
| `op-systemic` (L23 ll.21–106) | **Diabetic retinopathy as RETINAL DISEASE** — `sys-2` writes it as the systemic complication and the screening rule; the classification, grading and laser/anti-VEGF management belong to the retina chapter (`L19.1`) | `op-ret` | ☐ |
| `op-systemic` (L23 ll.107–210) | **Retinal vein occlusion and retinal artery macroaneurysm as RETINAL DISEASE** — written here only as hypertension's ocular consequences | `op-ret` | ☐ |
| `op-systemic` (L23 ll.247–267) | **The five stages of sickle proliferative retinopathy and their management** — ⚠️ these are **printed**, on `L19.2` lines 343–347, and were nearly tagged as absent; `sys-9` names the stages, the retinal chapter owns the management | `op-ret` | ☐ |
| `op-systemic` (L23) | **Central serous chorioretinopathy** | `op-ret` | ☐ **⚠️ ABSENT FROM ALL 27 DECKS — the receiving chapter will have to supply and tag it. Flagged now so this row is not read as a promise the material can keep** |
