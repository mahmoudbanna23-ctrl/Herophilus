...
| `op-cat` | **Cystoid macular oedema / Irvine–Gass as MACULAR DISEASE** | `op-ret` | ☐ |
...
| `op-uvea` `uv-8` — **RECONSTRUCTED 2026-08-18** | **CYSTOID MACULAR OEDEMA as retinal disease.** ⚠️ `uv-8` closes *"Cystoid macular oedema is `op-ret`’s — row filed."* **No such row exists** | `op-ret` | ☐ **already merged — reconciliation work.** Check `op-ret` and `op-ret-dr` for CMO before assuming it is owed; it may already be delivered, in which case tick rather than write |
