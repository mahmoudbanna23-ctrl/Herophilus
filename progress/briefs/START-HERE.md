...
| `op-intro` (**book ch.1**, printed 12–14) | **LID ANATOMY as anatomy** — the tarsal plates, **WHITNALL’S LIGAMENT**, **MÜLLER’S MUSCLE** as a named lid structure, and the **10 mm / 30 mm palpebral fissure dimensions**. `op-intro` writes the adnexal overview; the disease account is `op-lid`’s. ⚠️ **`Whitnall` returned ZERO across the module before this chapter was written** — grep re-run at merge, and the only two hits are `op-intro`’s own | `op-lid` | ☐ **already merged — reconciliation work.** `lid-1` carries the accessory glands and `lid-5`/`lid-6` the levator; the ligament and the fissure dimensions are the addition |
...
| `op-sclera` `scl-1` — **RECONSTRUCTED 2026-08-18** | **MARFAN SYNDROME and OSTEOGENESIS IMPERFECTA as systemic disease.** ⚠️ `scl-1` states *"a row is filed to `op-systemic`"*. **No such row exists.** Its evidence, as written: Marfan is named once module-wide (`cat-10`) and osteogenesis imperfecta nowhere | `op-systemic` | ☐ **already merged — reconciliation work.** ⚠️ **Re-run both greps before acting** — the evidence is dated and this module has already shipped one stale zero-hit claim (the GCA row) |
