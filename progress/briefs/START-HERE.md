...
| `op-va` (L3,4 sl.27) | **Hyphaema, hypopyon, cells & flare as DISEASES** — listed in `op-va` as anterior-chamber contents, which is what the slide is. The uveitis grading behind "cells & flare" is not on this deck | `op-uvea`, `op-trauma` | ☐ **NARROWED, verified pre-brief 2026-08-17 — the entities are written, the GRADING is not.** `hyphaema` (`va-14`, `trm-8`, `cat-10`, `red-3`, `red-4`), `hypopyon` (9 sections) and `cells & flare` (`va-14`, `orb-11`, `wht-6`, `red-4`, `red-8`) are all delivered, so `op-trauma`'s half is closed. **`grading of cells` returns ZERO module-wide** and every `SUN` hit in the file is the word "sun" inside another word, not the SUN uveitis grading — so the ONLY residue is the anterior-chamber cell/flare grade, and it can come from nowhere but `L13,14` |
...
| `op-white` (L33,34) | **Uveitis as a disease** | `op-uvea` | ☐ **⚠️ FOURTH row against this receiver, and `L13,14` extracts ZERO characters over 41 pp with no cache at all. `op-uvea` is the module's most blocked chapter — count its rows before briefing it.** |
