...
| `op-insid` (L35,36 ll.250–262) | **Macular and foveal anatomy as ANATOMY** | `op-ret` | ☐ **⚠️ receiver's own deck `L20` is still owed a visual read** |
| `op-insid` (L35,36 ll.369–408) | **Diabetic retinopathy as retinal disease** — FA/OCT, PRP, anti-VEGF, vitrectomy | `op-ret` | ☐ **⚠️ MERGE WITH `op-systemic`'s DR ROW ABOVE — two chapters opened the same row against different decks. `op-ret` writes it ONCE, against both.** Second time in the module two independent chapters have converged on one owed fact |
