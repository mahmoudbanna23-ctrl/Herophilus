...
| `op-systemic` (L23 ll.301–322) | **Metastases, lymphoma and leukaemic infiltrates as TUMOURS** — written here as the systemic malignancy link only | `op-onc` | ☐ **⚠️ receiver blocked on a visual read (`L24`, 78 pp)** ⚠️ **BLOCKER SUPERSEDED 2026-08-18 — the payload now comes from the BOOK, not the deck.** `ophthalmology.pdf` (HEALIX) **ch.17 Malignancies of the Eye and its Adnexa, printed 214–217 — FOUR pages** replaces `L24`’s **78 pages of `CamScanner`**. That is the largest single saving the ruling produced. The user ruled the book is the module’s main reference, so this row is deliverable and the deck read it was waiting on is not owed. |
