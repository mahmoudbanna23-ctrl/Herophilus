...
## 2026-09-20, s16 Pediatric Ophthalmology CLOSED -- 11 live, `questions.ophtho.ep.js` 417 -> 428; s20 staged

- `modules.js`: two new book-order groups, 16 Pediatric Ophthalmology (`op-peds`) and 20 Drugs and the Eye (`op-drugs`), chapters 154 -> 156, `dddb78f`; `sec-oph.js` rows 16 and 20 point at them.
- s16: 16 staged pairs (pdf 1118-1149), 9 boxed, 15 four-option and 1 five-option, no figure (the table on marked p.1119 sits inside the n1 box). Five folds, all within-bank exact with the same key, none takes `alsoIn` (`sec-oph.js` row 16 `folded: [9, 10, 13, 15, 16]`): n9 and n15 into `ophep-optics-refraction-9`, n10 into `ophep-cornea-11` (already carries the box), n13 into `ophep-malignancies-eye-adnexa-8`, n16 into `ophep-optics-refraction-18`. The four survivors gained a reprint citation in `source`. Drafted ids `ophep-pediatric-ophthalmology-1..8,11,12,14`, 2,306 words.
- The endpoint book prints option labels inside the option text on n11, n13, n16 (n11 prints 0. for D.); stripped on n11 in `array.js`, left on the folded rows. Printed as-is: n12 Pediatriac, RetinobU1stoma. Live `ophep-optics-refraction-18` option A reads Strabisn1us where s16 p.1148 prints Strabismus: look at p.194 before calling it a transcription fault.
- Route: probe first (work-text, work-vision, groq ok; big-pickle 403). Codex dispatched once, usage limit until 18:21; no capable gateway seat for an agentic job, so Sonnet lean-drafters (`ROUTE-OK`) built `array.js` + sweep and drafted from `oph-ep-s16-draft.brief.md` (a delta on the s13 brief). Checks: `val-oph-ep.js --part 1 16` ALL CHECKS PASSED, then a separate Sonnet refuter: byte-for-byte clean, three citation faults fixed before the splice (n1 alternating-fixation mechanism tagged, L29 slide 25 only shows it in photographs; n1 L18 slide 14 -> 15; n7 L18 slide 3 -> 2) plus a sibling cite in n7 (dark against the red reflex, L18 slide 6). L18 footers close a slide, so a line belongs to the NEXT footer number. The drafter appended a `module.exports` line that made the splice refuse (cannot isolate terminal array); removed.
- Tagged, no lecture states them (refuter grepped all files): ROP screening rule (n8, n14), ophthalmia neonatorum organisms and treatment (n2, n5), parts of n1, n4, n6, n7, n11, n12. No printed key contradicted.
- House: `oph-ep-p1-s16-pediatric-ophthalmology.house-collisions.md`, 3 drafted matches (n11, n12, n14) and 4 on the folded survivors.
