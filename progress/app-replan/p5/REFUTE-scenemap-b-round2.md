...
## Round 4 (2026-09-26) - owner rulings applied

**Verdict: PASS**, one minor check-count defect (non-blocking). Fit to commit.

1. Check + derivations. Real run: "60 passed", PORTRAIT crop [1066.0, 1445.4] (w 379.4), LAMP clips ~29.6, exit 0. scene-map-b.js has no 1445 / 29.6 / 379 literal; x0 is a getter returning SCENE_MAP.bookCluster.box[0]. Mutated copies in D:/tmp-scenemap-r2/brk/: bookCluster [1000,217,475,241] -> crop [1000.0,1379.4], lamp clip 95.6; lamp w 230 -> clip 17.6; viewport w 200 -> crop [1066.0,1260.5], exit 1 "book.blue ... not inside portrait crop". All derived.
   - DEFECT (minor): the "N passed" line prints BEFORE section 8, so the 4 book-in-portrait-crop asserts are never in the count (true total 64). The vw200 run printed "60 passed" and "1 FAILED" together. Exit code is still correct. Fix: move console.log(pass passed) below section 8.
2. Plate crops (r4-tablet.png, r4-pile.png, r4-lampbase.png, looked at). tablet [932,458,200,90]: bare parchment; left roll ends at x~932 (just clear); the roll's shadow darkens the top-left ~30 px - shadow, not an object. pile [1132,458,322,90]: bare parchment; lamp base sits directly above y=458 and does not enter the box, only its contact shadow darkens the top ~4 rows; right roll touches the top-right corner by ~2-4 px at x~1452-1454. No book, lamp or object intrudes. PASS.
3. Pile 322x90: legal but not a sane sprite slot. Aspect 3.6:1, and tablet+pile together cover 100% of the open scroll, hiding the parchment that makes it read as a scroll. Pile also sits under the lamp (x 1233-1454), so any sprite taller than 90 overlaps the lamp base. Recommend narrowing: pile ~[1160,458,200,90] (same size as tablet, 28 px gap after tablet, ends 1360, clear of the right roll). Or whatever size the sprite art turns out to be. Owner/design call, not a blocker.
4. Notes: rows for tablet (932,458,200,90) and pile (1132,458,322,90); "Ruling 1" and "Ruling 2" paragraphs with crop [1066.0, 1445.4] and the lamp clip; line 89 says "no OWNER-PENDING items remain". PASS. Nit: the drafter's pass is labelled "Round 3", which collides with this file's refuter round numbering.
