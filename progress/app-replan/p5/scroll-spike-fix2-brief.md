# P5 — scroll spike, fix round 2 (2026-09-26)
...
- Refuter round 2: `p5/REFUTE-scroll-spike-round2.md` — FAIL, 2 major + 8 minor (round 1 list: `p5/REFUTE-spike-round1.md`). Close EVERY defect; open
...
## Round 2 priorities (refuter round 2 wins over the list above where they differ)
- Long subject titles (Ophthalmology, Neuropsychiatry) must never run under the ring: at 1280, 390 and reduced motion. Let the title wrap or shrink; the ring keeps its fixed size.
- The rAF fallback must match the native ranges exactly: all three desk sprites, full plate pan, heading travel, shaft sweep. Drive both paths from ONE set of keyframes/ranges so they cannot drift.
- Candle glow sits ON the flame (measure the flame position on the plate), not on the wall.
- Reduced motion: sprites in their final, fully visible positions.
- NOTES.md: every claim must match the code; no "equivalent" unless it is.
